...
class LOOP extends CLOUD {
 ...
 void delete() { for (int i=p; i<n-1; i++) P[i].setTo(P[n(i)]); n--; p=p(p);} // deletes vertex p
 ...
 void insert(pt M, int p) { for (int i=n-1; i>p; i--) P[i+1].setTo(P[i]); n++; p=n(p); }
...
void setToConvexHull(CLOUD C) { // sets this loop to be the convex hull of the cloud C
  int hi=0, li=0; for (int i=0; i<C.n; i++) {if(C.P[i].y>C.P[hi].y) hi=i; else if(C.P[i].y<C.P[li].y) li=i; } // computes highest and lowest points
  pt A=P(C.P[li]), B=P(C.P[hi]); // renames them for convenience 
  int  [] L = new int [C.n-2], R = new int [C.n-2]; int cL=0, cR=0; // indices of vertices on the left and on the right of line (A,B)
  for (int i=0; i<C.n; i++) if (i!=li&&i!=hi) if(cw(A,B,C.P[i])) L[cL++]=i; else R[cR++]=i; // compute these indices
  n=0;  // resert this loop vertex counter
  P[n++]=P(A);  // add A to the convex hull
  bulgeConvexHull(B, A, R, cR); // bulges the right side of line (B,A) recursively
  P[n++]=P(B);   // add B to the convex hull
  bulgeConvexHull(A, B, L, cL); // bulges the right side of line (A,B) recursively
  }
  
void bulgeConvexHull(pt A, pt B, int [] V, int cV) { // bulges the right side of line (A,B) recursively to the convex hull of the cV vertices with indices in V
  if (cV==0) return; if (cV==1) {P[n++]=C.P[V[0]]; return; } // trivial cases with no or only 1 vertex
  vec AB=V(A,B), N=R(AB); int hi=0; for (int i=0; i<cV; i++) if( dot( V(A,C.P[V[i]]),N)  > dot( V(A,C.P[V[hi]]),N) ) hi=i; // index of furthest point
  pt M = P(C.P[V[hi]]); // furthest point
  int  [] L = new int [cV-1], R = new int [cV-1]; int cL=0, cR=0; // indices of vertices to the left of Line (M,B) and of thopse to the right of line (A,M)
  for(int i=0; i<cV; i++) if(i!=hi) if(cw(M,B,C.P[V[i]])) {L[cL++]=V[i];} else if(cw(A,M,C.P[V[i]])) {R[cR++]=V[i];}; // copute these indices
  bulgeConvexHull(M,B,L,cL); // recursive bulge in the left ear
  P[n++]=M;                  // add M to the convex hull
  bulgeConvexHull(A,M,R,cR); // recursive bulge in the right ear
  }

// ************************************** DELAUNAY MESH *********************************

void showMesh(CLOUD C) { // sets this loop to be the convex hull of the cloud C
  Boolean [] used = new Boolean [C.n]; // keeps track of which vertices have been used
  int hi=0, li=0; for (int i=0; i<C.n; i++) {if(C.P[i].y>C.P[hi].y) hi=i; else if(C.P[i].y<C.P[li].y) li=i; } // computes highest and lowest points
  used[li]=true; used[hi]=true;
  pt A=P(C.P[li]), B=P(C.P[hi]); // renames them for convenience 
  int  [] L = new int [C.n-2], R = new int [C.n-2]; int cL=0, cR=0; // indices of vertices on the left and on the right of line (A,B)
  for (int i=0; i<C.n; i++) if (i!=li&&i!=hi) if(cw(A,B,C.P[i])) L[cL++]=i; else R[cR++]=i; // compute these indices
  n=0;  // resert this loop vertex counter
  P[n++]=P(A);  // add A to the convex hull
  bulgeConvexHull(B, A, R, cR, used); // bulges the right side of line (B,A) recursively
  P[n++]=P(B);   // add B to the convex hull
  bulgeConvexHull(A, B, L, cL, used); // bulges the right side of line (A,B) recursively
  for(int i=0; i<C.n; i++) if (used[i]) show(C.P[i]);
  }
  
void bulgeConvexHull(pt A, pt B, int [] V, int cV, Boolean [] used) { // bulges the right side of line (A,B) recursively to the convex hull of the cV vertices with indices in V
  if (cV==0) return; if (cV==1) {P[n++]=C.P[V[0]]; return; } // trivial cases with no or only 1 vertex
  vec AB=V(A,B), N=R(AB); int hi=0; for (int i=0; i<cV; i++) if( dot( V(A,C.P[V[i]]),N)  > dot( V(A,C.P[V[hi]]),N) ) hi=i; // index of furthest point
  pt M = P(C.P[V[hi]]); // furthest point
  used[hi]=true;
...
void showDelaunayOfPoints(float o) { 
      for (int i=0; i<n-2; i++)  for (int j=i+1; j<n-1; j++)  for (int k=j+1; k<n; k++)  {
                   ...
                   if (!found) {if(contains(A(P[i],P[j],P[k]))) fill(green); else fill(yellow); show(P[i],P[j],P[k],o); };                   //                  if (cw(P[i],P[n(i)],P[j])) {}
